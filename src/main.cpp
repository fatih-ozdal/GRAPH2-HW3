...
void MouseMoveCallback(GLFWwindow* wnd, double x, double y)
{
    ...
    if(state.altActive)
    {
        state.yaw += float(diffX * DefaultSensitivity);
        state.pitch += float(diffY * DefaultSensitivity);
    }

    if(state.mouseToggle)
    {
        // X Rotation
        glm::vec3 lookDir = state.cam.gaze - state.cam.pos;
        glm::mat4 rotateX = glm::rotate(glm::identity<glm::mat4>(),
                                        float(-diffX * DefaultSensitivity),
                                        state.cam.up);
        glm::vec3 rotated = rotateX * glm::vec4(lookDir, 1.0f);
        state.cam.pos = state.cam.gaze - rotated;
        // Y Rotation
        lookDir = state.cam.gaze - state.cam.pos;
        glm::vec3 left = glm::normalize(glm::cross(state.cam.up, glm::normalize(lookDir)));
        glm::mat4 rotateY = glm::rotate(glm::identity<glm::mat4>(),
                                        float(diffY * DefaultSensitivity), left);
        rotated = rotateY * glm::vec4(lookDir, 1.0f);
        state.cam.pos = state.cam.gaze - rotated;

        // Redefine up
        // Enforce an up vector which is orthogonal to the xz plane
        state.cam.up = glm::cross(rotated, left);
        state.cam.up[2] = 0.0f;
        state.cam.up[0] = 0.0f;
        state.cam.up = glm::normalize(state.cam.up);
    }
    state.prevMouseX = x;
    state.prevMouseY = y;
}
...
void MouseScrollCallback(GLFWwindow* wnd, double dx, double dy)
{
    ...
    if(look.length() > 0.1f)
    {
        state.cam.pos += float(dy * DefaultZoomPercentage) * look;
    }
}
...
void KeyboardCallback(GLFWwindow* wnd, int key, int scancode, int action, int modifier)
{
    ...
    glm::vec3 look = glm::normalize(state.cam.gaze - state.cam.pos);
    glm::vec3 left = glm::normalize(glm::cross(state.cam.up, look));
    if(key == GLFW_KEY_W)
    {
        //state.cam.pos += ratio[0] * look;
        //state.cam.gaze += ratio[0] * look;
        state.speedModifier += ratio[0];
    }
    if(key == GLFW_KEY_S)
    {
        state.speedModifier -= ratio[0];
    ...
    if(key == GLFW_KEY_A)
    {
        state.cam.pos += ratio[0] * left;
        state.cam.gaze += ratio[0] * left;
    }
    if(key == GLFW_KEY_D)
    {
        state.cam.pos -= ratio[0] * left;
    ...
    {
        if(action == GLFW_PRESS)    state.altActive = true;
        if(action == GLFW_RELEASE)  state.altActive = false;
    }
